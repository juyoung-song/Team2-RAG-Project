...
사용자가 한눈에 정보를 파악할 수 있도록 반드시 아래 형식을 엄격히 준수하십시오.

**ANSWER**: [질문에 대한 답변을 '한 줄'로 명료하게 요약 (한글)]

**상세 부연 설명**
1. **근거**: [문서에서 찾은 직접적인 근거 문장]
2. **핵심 수치/조건**: [금액, 기간, 인원 등 가장 중요한 수치 데이터]
3. **참고사항**: [추가로 주의해야 하거나 알아야 할 관련 정보]
...
def build_chain(retriever, model_name="gpt-5-mini", temperature=1, prompt_template=CUSTOMIZED_PROMPT):
    ...
    # 3. 최적화 핵심: 검색된 context를 버리지 않고, 최종 딕셔너리에 'answer'라는 키로 덧붙임
    return retrieval.assign(answer=answer_generation)
    

def ask(chain, question, use_langfuse=False):
    """
    질문에 대한 답변(str)과 참고한 문서 리스트(List[Document])를 함께 반환합니다.
    """
    if use_langfuse:
